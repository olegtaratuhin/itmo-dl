...
def smooth(mat):
    ...
    flatSimp = [Counter(getVicinVals(mat, x, y, 4)).most_common(1)[0][0] for y in range(0, height) for x in
                range(0, width)]
...
def getLabelLoc(mat, region):
    ...
    best = 0
    for i in range(0, len(region['x'])):
        goodness = sameCount(mat, region['x'][i], region['y'][i], -1, 0) * sameCount(mat, region['x'][i],
                                                                                     region['y'][i], 1, 0) * sameCount(
            mat, region['x'][i], region['y'][i], 0, -1) * sameCount(mat, region['x'][i], region['y'][i], 0, 1)
        if goodness > best:
            best = goodness
            bestI = i
